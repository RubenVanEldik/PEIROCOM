...
def fit_curve(independant_variables, dependent_variables, *, function, return_parameters=False, num_points=200):
    """
    Fit a curve for a given series and function
    """
    assert validate.is_series(independant_variables)
    ...
    parameters, covariance = scipy.optimize.curve_fit(function, independant_variables, dependent_variables, maxfev=10 ** 4)

    # Generate the curve
    curve_index = np.linspace(independant_variables.min(), independant_variables.max(), num_points)
